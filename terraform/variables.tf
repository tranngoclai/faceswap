...
variable "rp_r2_access_key_id" {
  description = "Cloudflare R2 access key ID (RCLONE_CONFIG_R2_ACCESS_KEY_ID on the worker)"
  type        = string
  sensitive   = true
  default     = ""
}

variable "rp_r2_secret_access_key" {
  description = "Cloudflare R2 secret access key (RCLONE_CONFIG_R2_SECRET_ACCESS_KEY on the worker)"
  type        = string
  sensitive   = true
  default     = ""
}

variable "rp_r2_endpoint" {
  description = "Cloudflare R2 S3-compatible endpoint URL (RCLONE_CONFIG_R2_ENDPOINT on the worker)"
  type        = string
  default     = ""
}

variable "rp_r2_bucket" {
  description = "Cloudflare R2 bucket name (R2_BUCKET on the worker)"
  type        = string
  default     = "faceswap"
}
